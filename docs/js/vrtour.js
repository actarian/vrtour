...
/*#__PURE__*/
function (_EmittableGroup) {
  _inherits(Views, _EmittableGroup);

  function Views(parent) {
    var _this;
    ...
    var floor = _this.floor = _this.addFloor(_assertThisInitialized(_this));

    var ceil = _this.ceil = _this.addCeil(_assertThisInitialized(_this));

    var points = _this.points = _this.addPoints(_assertThisInitialized(_this));
  ...
  _createClass(Views, [{
    ...
    value: function onInitView(previous, current) {
      ...
      this.onExitPoints(previous).then(function () {
        ...
        _this2.onExitView(previous).then(function () {
          // if (!previous) {
          _this2.onEnterView(current).then(function () {
            _this2.onEnterPoints(current); // console.log(this.points.vertices);
        ...
        if (view) {
          setTimeout(function () {
            ...
            loader.load(view.image, function (texture) {
              ...
              */
              var material = _this4.room.sphere.material;
              material.opacity = 0;
    ...
    value: function removePoint(i) {
      ...
      return new Promise(function (resolve, reject) {
        var point = _this8.points.children[i];
        var from = {
          opacity: 1
        };
        TweenMax.to(from, 0.7, {
...
var OFF = new THREE.Color(0xcccccc);
...
function (_Controller) {
  ...
  _createClass(OculusQuestController, [{
    ...
    value: function addModel(hand) {
      ...
      var matcap = new THREE.TextureLoader().load('img/matcap.jpg');
      var texture = new THREE.TextureLoader().load("".concat(path, ".png"));
      var material = new THREE.MeshMatcapMaterial({
        color: OFF,
      ...
      var i = 0;
      loader.load("".concat(path, ".obj"), function (object) {
        var x = hand === _gamepads.GAMEPAD_HANDS.RIGHT ? -(0, _const.cm)(1) : (0, _const.cm)(1);
        object.traverse(function (child) {
          if (child instanceof THREE.Mesh) {
            child.material = material.clone(); // 0 joystick, 1 trigger, 2 grip, 3 Y, 4 X
            // 0 joystick, 1 trigger, 2 grip, 3 A, 4 B

            function onButton(value) {
              child.position.set(0, -value * (0, _const.mm)(5), 0);
            ...
            switch (child.name) {
              case 'joystick':
                _this.buttons[0] = onButton;

                _this.move = function (axis) {
                  child.rotation.set(axis.y * (0, _const.deg)(20), 0, axis.x * (0, _const.deg)(20));
                };
              ...
              case 'trigger':
                _this.buttons[1] = function (value) {
                  child.rotation.set(-value * (0, _const.deg)(10), 0, 0);
                  child.material.color.set(OFF.lerp(ON, value));
                };
              ...
              case 'grip':
                _this.buttons[1] = function (value) {
                  child.position.set(-value * (0, _const.mm)(5), 0, 0);
                  child.material.color.set(OFF.lerp(ON, value));
                };
              ...
              case 'buttonY':
              case 'buttonA':
                _this.buttons[3] = onButton;
                break;

              case 'buttonX':
              case 'buttonB':
                _this.buttons[4] = onButton;
                break;
...
function (_Emittable) {
  ...
  function Controllers(renderer, scene, pivot) {
    ...
    var text = _this.text = _this.addText_(pivot);
  ...
  _createClass(Controllers, [{
    ...
    value: function addGamepads_() {
      ...
      gamepads.on('connect', function (gamepad) {
        ...
        if (gamepad.hand === _gamepads.GAMEPAD_HANDS.LEFT) {
          ...
          var menu = _this3.menu = new _menu.default(controller);
          menu.on('down', function (event) {
            _this3.onMenuDown(event);
          });
        } else {
    ...
    value: function addTestController_() {
      if (_const.TEST_ENABLED) {
        ...
        var controller = new _oculusQuestController.default(this.scene, _gamepads.GAMEPAD_HANDS.RIGHT);
...
function () {
  ...
  _createClass(VRTour, [{
    ...
    value: function init() {
      ...
      var vr = this.vr = this.addVR(renderer, container); // this.addIO();
      ...
      if (vr.mode !== _vr.VR_MODE.NONE) {
        ...
        var dragListener = this.dragListener = orbit.setDragListener(container);
      } else {
        camera.target.z = _const.ROOM_RADIUS;
        camera.lookAt(camera.target);

        var _orbit = this.orbit = new _orbit2.default();

        var _dragListener = this.dragListener = _orbit.setDragListener(container);
      } // raycaster
    ...
    value: function addIO() {
      ...
      var ims = this.ims = new Array(10).fill(null).map(function (x) {
        var im = new _interactive.default();
    ...
    value: function onMouseDown(event) {
      ...
      try {
        ...
        if (event.shiftKey) {
          var intersections = raycaster.intersectObjects(this.pivot.room.children);
